...
* After I found a combination of dataset that works 98% correctly, I stopped the data collection (as more data are not always helpful) and used the twice of the dataset to retrain the model.
...
### Results
I used the final dataset which combined Udacity's and my own, and included a total of 59,664 data points. I finally chose to implement only one 50% dropout after the first fully connected layer, so the final data samples are    and these data are remained after distribution flattening, and this set was further split into a training set of 16,482 (80%) data points and a validation set of 868 (20%) data points. 

The total params: 252,219, trainable params: 252,219, non-trainable params: 0. The kernel regularizer for the model's each layer and the one dropout works fairly good to control overfitting. The final training loss is    , validation loss is ..... These were not the lowest, but the model with these weights performed the best on the autonomous driving in the sitmulator.

### Conclusion and Discussion
I am excited to see the vehicle can drive autonomously and smoothly through my trained model. Through the many trial-and-error process involved many hours, I was able to develop a deeper understanding about the relationship between the training data as human's driving behaviors and the predicted vehicle cloning behaviors, which is essential to this project. In summary, I have the following insights:
* When we collect the human driving behavior data, we shall show a strong intension of our behaviors. Which means when we drive on a sharp left curve, our intention is to steer our wheele along the left curve to stay left as closely as possible so that our car would not be thrown out of the road to the right side. This reflects to the data that there shall be many steering angles in the dataset.
* In the training model on the emulator, speeding up so that we will collect as many as discovery data as we can.
* During data collection, the transaction period of time from a straight line road condition to sharpe curves is essential to show our driving behavioral intention. For example, when we see a left curve ahead from a straight lane line, we shall adjust our training vehicle gradually to the left to prepare for the left curve. This type of data will be shown in the autonomous mode during the model improvement process, then we will know what other data we will need to feed in the model to improve it.
* Collecting good data is key in this project, as we have the NVIDA's CNN model or else models available to start. But fine tuning and testing with varied model architecutre to find the most effective model architecture is interesting to me in the next phase.
* Very small validation loss value does not correlate with a good model in this project. But through teniques of avoiding overfitting and ensuring a good training data, when we see the model is converging, we have a good base to guess that the model would work.
...
* I also provided the project folder name [/data](   )
